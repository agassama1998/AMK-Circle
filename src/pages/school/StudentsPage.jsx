...
import { Plus, Search, Edit2, Eye, Save, BookOpen, Users, Activity, AlertCircle, Trash2, CheckCircle } from 'lucide-react'
...
export default function StudentsPage() {
  ...
  // ── Delete ────────────────────────────────────────────────────────────────────
  const openDeleteModal = (s) => { setDeleteTarget(s); setDeleteMsg('') }

  const confirmDelete = async () => {
    setDeleteLoading(true)
    const r = await window.api.students.delete({ id: deleteTarget.id, orgId })
    ...
    <div className="space-y-5 animate-fade-in">
      ...
      >
        ...
        <div className="flex items-start gap-3 p-3 bg-red-50 dark:bg-red-900/20 rounded-xl mb-4">
          ...
          <div>
            ...
            <p className="text-xs text-red-600 dark:text-red-300">
              All attendance records, Quran progress, grades, and boarding assignments for this student will be permanently deleted.
            </p>
          </div>
        </div>
        <p className="text-sm text-gray-600 dark:text-gray-400">
          ...
          {deleteTarget?.student_id ? ` (${deleteTarget.student_id})` : ''}?
        </p>
      </Modal>
